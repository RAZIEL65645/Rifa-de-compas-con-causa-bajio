...
luckyBtn.addEventListener('click', ()=>{
  ...
  if(clienteCiudad) clienteCiudad.value = '';
  if(clienteEstado) clienteEstado.value = '';
  luckPicked = [];
});
...
// Mostramos el formulario al dar click en LOS QUIERO (maquinita)
luckConfirmBtn.onclick = function() {
  if(!luckPicked.length) return;
  datosWspForm.style.display = 'block';
};
// Si el usuario abre la maquinita de nuevo, oculta el form y limpia campos
luckyBtn.addEventListener('click', ()=>{
...
// Al hacer click en enviar a WhatsApp (maquinita)
enviarWspBtn.onclick = function() {
  const nombre = clienteNombre.value.trim();
  const tel = clienteTelefono.value.trim();
  const estado = clienteEstado.value;
  if(nombre.length < 5 || tel.length < 7 || ciudad.length < 2 || !estado) {
...
30 POR $477
———————————— 
⯯*CUENTAS DE PAGO AQUÍ:* www.rifadecompasconcausabajíocom/pagos
*Celular:* 4761328415
...
// --------- APARTADO MANUAL DE BOLETOS SELECCIONADOS ---------
function updateApartadoManualBtn() {
  const boton = document.getElementById('apartado-manual');
  const form = document.getElementById('form-manual-wsp');
  if(cart.length > 0){
...
// Click en "APARTAR ESTOS BOLETOS"
document.getElementById('btn-apartar-manual').onclick = function() {
  document.getElementById('form-manual-wsp').style.display = "block";
  document.getElementById('manual-nombre').value = '';
  document.getElementById('manual-telefono').value = '';
  document.getElementById('manual-ciudad').value = '';
  document.getElementById('manual-estado').value = '';
};

// Enviar seleccionado manualmente a WhatsApp
document.getElementById('enviar-manual-wsp-btn').onclick = function() {
  const nombre = document.getElementById('manual-nombre').value.trim();
  const tel = document.getElementById('manual-telefono').value.trim();
  const ciudad = document.getElementById('manual-ciudad').value.trim();
  const estado = document.getElementById('manual-estado').value;
  if(nombre.length < 5 || tel.length < 7 || ciudad.length < 2 || !estado) {
    alert('Por favor llena todos los campos correctamente.');
    return;
  }
  const boletos = cart.length;
  const mensaje = `Hola, Aparte números para FORD LOBO PLATINUM!!

———————————— 
🎟 *${boletos} BOLETOS:* *${cart.join('* ')}*
*Nombre:* ${nombre} 
*Teléfono:* ${tel}
*Ciudad:* ${ciudad}
*Estado:* ${estado}
...
El siguiente paso es enviar foto del comprobante de pago por aquí`;
  const wspurl = "https://wa.me/524761328415?text=" + encodeURIComponent(mensaje);
  window.open(wspurl, "_blank");
  // Limpia carrito y formulario si deseas
  cart = [];
  renderNumbers(visibleNumbers);
  updateApartadoManualBtn();
  document.getElementById('form-manual-wsp').style.display = "none";
  document.getElementById('manual-nombre').value = '';
  document.getElementById('manual-telefono').value = '';
  document.getElementById('manual-ciudad').value = '';
  document.getElementById('manual-estado').value = '';
